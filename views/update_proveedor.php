...
<div class="container py-4" style="max-width:600px; margin:0 auto;">
    ...
    <h2 class="titulo mb-4">Actualizar Proveedores</h2>
    ...
    <form action="index.php" method="POST" class="mb-4">
        <input type="hidden" name="action" value="searchProveedorForUpdate">
        <div>
            <label style="color:#d4af37; font-weight:600; display:block; margin-bottom:6px;">
                Buscar por Nombre:
            </label>
            <input type="text" name="nombreProveedor" placeholder="Ingrese nombre" required
            ...
            <button type="submit" class="btn-dashboard">Buscar Proveedor</button>
        </div>
    </form>

    <?php if (isset($_POST['nombreProveedor']) && empty($proveedores)): ?>
        <div class="mb-4 p-3" style="background:#3a1a1a; color:#f44336; border:1px solid #f44336; border-radius:8px;">
            <strong>✗ No se encontró ese proveedor.</strong>
        </div>
    <?php endif; ?>
    ...
    <?php if (!empty($proveedores)): ?>
        ...
        <div class="mb-4 p-4" style="background:#181818; border:1px solid #d4af37; border-radius:12px;">
            <form action="index.php" method="post">
                ...
                <input type="hidden" name="NITproveedor" value="<?= htmlspecialchars($proveedor['NITproveedor']) ?>">

                <div class="mb-3">
                    <label style="color:#d4af37; font-weight:600; display:block; margin-bottom:6px;">NIT Proveedor:</label>
                    <input type="text" name="NITproveedor" value="<?= htmlspecialchars($proveedor['NITproveedor']) ?>"
                        readonly
                        style="background:#2a2a2a; color:#aaa; border:1px solid #555; border-radius:7px; padding:8px 10px; width:100%;">
                </div>

                <div class="mb-3">
                    <label style="color:#d4af37; font-weight:600; display:block; margin-bottom:6px;">Nombre Proveedor:</label>
                    <input type="text" name="nombreProveedor" value="<?= htmlspecialchars($proveedor['nombreProveedor']) ?>"
                        required
                        style="background:#232323; color:#fff; border:1px solid #d4af37; border-radius:7px; padding:8px 10px; width:100%;">
                </div>

                <div class="mb-3">
                    <label style="color:#d4af37; font-weight:600; display:block; margin-bottom:6px;">Dirección:</label>
                    <input type="text" name="direcProveedor" value="<?= htmlspecialchars($proveedor['direcProveedor']) ?>"
                        style="background:#232323; color:#fff; border:1px solid #d4af37; border-radius:7px; padding:8px 10px; width:100%;">
                </div>

                <div class="mb-3">
                    <label style="color:#d4af37; font-weight:600; display:block; margin-bottom:6px;">Teléfono:</label>
                    <input type="text" name="telefono" value="<?= htmlspecialchars($proveedor['telefono']) ?>"
                        style="background:#232323; color:#fff; border:1px solid #d4af37; border-radius:7px; padding:8px 10px; width:100%;">
                </div>

                <button type="submit" class="btn-dashboard w-100">Actualizar Proveedor</button>
            </form>
        </div>
        <?php endforeach; ?>
    ...
    <?php else: ?>
        ...
        <div class="mb-4 p-3 d-flex align-items-center gap-3"
            ...
            <span style="font-weight:600;">Ingresa un nombre para buscar y editar un proveedor.</span>
        </div>
        </center>
    <?php endif; ?>
