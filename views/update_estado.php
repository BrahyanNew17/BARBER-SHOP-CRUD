...
<div class="container py-4" style="max-width:600px; margin:0 auto;">
    ...
    <h2 class="titulo mb-4">Actualizar Estados</h2>
    ...
    <form action="index.php" method="POST" class="mb-4">
        <input type="hidden" name="action" value="searchEstadoByName">
        <div>
            <label style="color:#d4af37; font-weight:600; display:block; margin-bottom:6px;">
                Buscar por Estado:
            </label>
            <input type="text" name="estado" placeholder="Ingrese estado" required
            ...
            <button type="submit" class="btn-dashboard">Buscar Estado</button>
        </div>
    </form>

    <?php if (isset($_POST['estado']) && empty($estados)): ?>
        <div class="mb-4 p-3" style="background:#3a1a1a; color:#f44336; border:1px solid #f44336; border-radius:8px;">
            <strong>✗ No se encontró ese estado.</strong>
        </div>
    <?php endif; ?>
    ...
    <?php if (!empty($estados)): ?>
        ...
        <div class="mb-4 p-4" style="background:#181818; border:1px solid #d4af37; border-radius:12px;">
            <form action="index.php?action=actualizarEstado" method="post">
                <input type="hidden" name="action" value="actualizarEstado">
                <input type="hidden" name="idEstado" value="<?= htmlspecialchars($estado['idEstado']) ?>">

                <div class="mb-3">
                    <label style="color:#d4af37; font-weight:600; display:block; margin-bottom:6px;">Estado:</label>
                    <input type="text" name="estado" value="<?= htmlspecialchars($estado['estado']) ?>"
                        required
                        style="background:#232323; color:#fff; border:1px solid #d4af37; border-radius:7px; padding:8px 10px; width:100%;">
                </div>

                <button type="submit" class="btn-dashboard w-100">Actualizar Estado</button>
            </form>
        </div>
        <?php endforeach; ?>
    ...
    <?php else: ?>
        ...
        <div class="mb-4 p-3 d-flex align-items-center gap-3"
            ...
            <span style="font-weight:600;">Ingresa un estado para buscarlo y editarlo.</span>
        </div>
        </center>
    <?php endif; ?>
